...
| Theorems 3, 4 | rigorous given Lemma 1; finite-L corrections are visible in exact computation (Section 9); the silence prefactor in Theorem 4 is off by about a factor of two and is downgraded to a scaling law |
...
**Exact branching process (application of Theorems 3 and 4), from `results/theorems/verification.md`.** With the exact generating-function recursion and no approximation: the throughput gap times depth rises through 0.80, 1.04, 1.23, 1.38, 1.47, 1.52 at L = 2 to 64 and reaches 1.5875 at L = 1024 against x* = 1.5936, converging from below. The one-shot gap at λ = 10⁻³ is 0.043, 0.038, 0.036, 0.037 at L = 2, 4, 8, 16, flat in depth as proved; the ratio of gaps for a fourfold cost is 2.004 and 2.014 at two cost levels against the proved 2.0. The constant √(λv) underestimates the exact gap by 15 to 35 percent, which is the accuracy of Lemma 1 at δ ≈ 0.04: the near-critical law overestimates exact survival by 5 to 10 percent at δ = 0.01, by 15 to 25 percent at δ = 0.03, and by 40 to 70 percent at δ = 0.1. The silence threshold scales with cost as proved, L_s ∝ λ^(−1/2), with onset ratios of 2.06, 2.13, 2.29 per fourfold cost against 2.0, but the prefactor is wrong: exact onsets are at L = 70, 34, 16, 7 for λ = 2.5×10⁻⁴, 10⁻³, 4×10⁻³, 1.6×10⁻² against predicted 126, 63, 32, 16. See opposing evidence, item 4.

**Leaky integrator (application of Theorem 5).** Simulated with a least-squares readout over 120,000 steps and a grid of 190 radii: the optimal radius squared is 0.497, 0.748, 0.874, 0.941, 0.970 at k = 2, 4, 8, 16, 32 against the proved (k − 1)/k = 0.500, 0.750, 0.875, 0.938, 0.969, and it does not move across noise standard deviations of 0.01, 0.3, and 1.0. Record: the first run of the check matched k/(k + 1) instead, because the script's delay index was shifted by one relative to the theorem's convention. That was a script error, corrected and noted here, not evidence.
...
4. The silence threshold in Theorem 4 has the right scaling and the wrong constant. Exact onsets are about 0.53 of the predicted L_s at every cost level tested. The derivation compares the value 2I₀/(vL) at the leading-order optimum with the cost λ/δ*, and both ingredients are off at moderate δ: Lemma 1 overestimates survival there, and the true optimum has already moved by the time silence sets in. The claim that survives is L_s = κ/√(λv) with κ ≈ 1.06 measured rather than the derived 2. A derivation that recovers κ is owed; until then Theorem 4's threshold is a scaling law with an empirical prefactor.

None of these four is fatal to the structure of Section 7, but each one limits a specific claim, and the second and third are precisely where a confirmatory run could kill Section 7 (iii) or (iv) for the population system. If that happens, it will be recorded as such.

## 10. Addendum for version two: what the population measurement actually predicts

**The unit error in applying Theorem 6.** Theorem 6 counts coverage in groups and gives a constant ln G. The quantity measured in the population studies is the growth of the number of *agents* holding the eventual best genotype, and the window edge is defined by mean-agent fitness, which depends on how many agents hold the best, not how many groups. Version one therefore applied the theorem with the wrong unit, and its ln G prediction (P-B5) was not supported. The corrected statement follows.

**Proposition 8 (edge location from exponential spread).** In a population of N agents, let the eventual best genotype first appear at time t₀ and let the number of agents holding it grow geometrically with per-step factor σ until saturation. Let the mean-agent performance window's left edge be the smallest communication rate at which mean fitness is within τ of the plateau, and let the plateau exceed the isolated mean by Δ. Then, at the edge, the fraction of agents holding the best at the horizon is C_req = 1 − τ/Δ, and the measured multiplier satisfies σ^(T − t₀) ≈ C_req N, so that for a small gap

    (σ − 1) · (T − t₀) ≈ ln(C_req · N).

*Derivation.* Mean fitness at T is C·plateau + (1 − C)·μ up to the reached agents' own endpoints, so the shortfall τ corresponds to C_req = 1 − τ/Δ. Geometric growth from one holder at t₀ to C_req N holders at T gives σ^(T − t₀) = C_req N; take logarithms and use ln σ ≈ σ − 1. ∎

Everything on the right side is measured independently of the holders trajectory: N and T are set, t₀ is the first-appearance time, τ is the window tolerance, and Δ comes from fitness at the plateau and at m = 0. The left side is measured from holders growth. The proposition therefore predicts the gap at the edge with no free parameter, predicts that the gap scales as 1/(T − t₀) rather than 1/T, and predicts that gap × (T − t₀) rises by ln 2 for every doubling of N at fixed everything else. None of those three appears in version one's data as a tested statement; the third contradicts version one's ln G expectation (which predicted no N dependence at fixed G). Retrospective check, which counts for nothing: version one's gap × T at K = 6, N = 100 was 6.2, 7.4, 23.4, 5.3, 6.1, while ln(C_req N) with Δ ≈ 0.06 and τ = 0.005 is ln(92) ≈ 4.5, and the (T − t₀)/T correction raises the predicted gap × T to roughly 5 to 7.

**Reservoir: the ridge penalty as effective noise.** In a linear reservoir with a ridge readout, the regularizer adds λ_ridge to the diagonal of the state covariance in the readout's normal equations, which is the same role played by isotropic input noise of variance proportional to λ_ridge. The conjecture for version two is therefore that at negligible input noise the exponent rises with the ridge penalty exactly as it rises with noise, and that in the noise-dominated limit the exponent approaches 1 with constant 1/2 as in Theorem 5. Both are stated as predictions, not theorems.

**Design-pilot correction to Proposition 8 (seed base 5151, before the version-two freeze).** The zero-parameter prediction held within a factor of 1.5 in 14 of 16 cells, with every estimate inside its bootstrap interval. The slope of gap × (T − t₀) against ln N came out near 0.5, not 1. The reason is in the measurement, not in the coverage argument: inside an island the best genotype spreads to all s = N/G members in one step, so the agent-holder count jumps by ln s before cross-island spread begins, and a least-squares line through the whole window dilutes that jump. The agent-holder multiplier therefore mixes two rates. The clean statement of Theorem 6 for this system counts islands: with ε the per-step cross-island growth of the number of islands holding the best,

    ε · (T − t₀) ≈ ln(G · C_req),

independent of N at fixed G, rising as ln G at fixed N. Version two therefore measures both quantities. The agent-holder ratio test (Proposition 8) stays, since it held; the island-count form carries the N-independence and ln G predictions, which version one's P-B5 tested with the wrong quantity.

**Second design-pilot correction: the coverage law is logistic.** With the island count recorded, the exponential-saturation constant of Theorem 6, ln(C_req G) ≈ 2.2 for ten islands, was too small by a factor of two to four in every cell of the pilot. Spread among a finite set of units is logistic: with I(t) islands holding the best and per-step cross-island rate ε, dI/dt = ε I (1 − I/G), so the time from one island to coverage C is ln((G − 1) C/(1 − C))/ε. For G = 10 and C_req = 0.93 that is 4.8/ε, and the pilot's median for ε × (T − t₀) was about 5.4. Theorem 6 as stated in Section 6 assumed C(ε, T) = 1 − exp(−(εT − ln G)), which is the logistic law's large-time tail only; the correct statement of its constant is ln((G − 1) C_req/(1 − C_req)), which still rises as ln(G − 1) and is independent of N. The same law gives a timing prediction that avoids fitting growth at all: at the edge, half coverage is reached at a fraction ln(G − 1)/ln((G − 1) C_req/(1 − C_req)) of the available time. Version two freezes both.

**Estimator note for version three.** Version two's population tests were each turned inconclusive by one cell whose plateau maximum was an inflated grid point. The plateau is now the median of the upper half of the smoothed curve. The predictions are unchanged.
